...
#Uses the CodeQL CLI
#act --action-offline-mode --input sample=node-sample -s GITHUB_TOKEN=$(gh auth token) -W '.github/workflows/codeql-cli.yaml'
...
act --action-offline-mode --input language=javascript --input query_suite=security-extended -s GITHUB_TOKEN=$(gh auth token) -W '.github/workflows/codeql.yaml'